...
class CallTracerCLI extends ProfilerCLI {
    ...
    private static void printCSV(PrintStream out, CallTracer tracer) {
        ...
        for (Map.Entry<Integer, Integer> entry : callMapCount.entrySet()) {
            ...
            String sourceNode = callMapKey.get(mapKey).sourceRootNode.toString().replace(",",";");
            String targetNode = callMapKey.get(mapKey).targetRootNode.toString().replace(",",";");
            if(callMapKey.get(mapKey).sourceSourceSection != null) {
                nameSource = callMapKey.get(mapKey).sourceSourceSection.getSource().getName().replace(",", ";");
                int startLineSource = callMapKey.get(mapKey).sourceSourceSection.getStartLine();
                int endLineSource = callMapKey.get(mapKey).sourceSourceSection.getEndLine();
                int startColumnSource = callMapKey.get(mapKey).sourceSourceSection.getStartColumn();
                int endColumnSource = callMapKey.get(mapKey).sourceSourceSection.getEndColumn();
                sourceLocation = nameSource + "~" + startLineSource + "-" + endLineSource + ":" + startColumnSource + "-" + endColumnSource;
            }
            else{
                nameSource="null";
                sourceLocation="null";
            }
            if (callMapKey.get(mapKey).targetSourceSection != null) {
                nameTarget = callMapKey.get(mapKey).targetSourceSection.getSource().getName().replace(",",";");
                int startLineTarget = callMapKey.get(mapKey).targetSourceSection.getStartLine();
                int endLineTarget = callMapKey.get(mapKey).targetSourceSection.getEndLine();
                int startColumnTarget = callMapKey.get(mapKey).targetSourceSection.getStartColumn();
                int endColumnTarget = callMapKey.get(mapKey).targetSourceSection.getEndColumn();
                targetLocation = nameTarget + "~" + startLineTarget + "-" + endLineTarget + ":" + startColumnTarget + "-" + endColumnTarget;
            }
            else{
                nameTarget="null";
                targetLocation="null";
            ...
            if (tracer.usingTime()){
            	ArrayList<Long> excecutionTime = callMapTimes.get(mapKey);
                String delimitedList = excecutionTime.stream()
                        .map(Object::toString)
                        .collect(Collectors.joining("; "));
                temp = temp + "[" + delimitedList + "]" + ",";
            }
            if (tracer.usingArguments()){
                ArrayList<String> printArguments = callMapKey.get(mapKey).arguments;
                String delimitedList = printArguments.stream()
                        .map(Object::toString)
                        .collect(Collectors.joining("; "));
                temp = temp + "[" + delimitedList + "]" + ",";
            }
            if (tracer.usingResults()){
                String returnResults = callMapKey.get(mapKey).result.toString().replace(",",";");
                temp = temp + returnResults + ",";
            }
            if (tracer.usingObjectInstances()){
                String objectInstance = callMapKey.get(mapKey).objectInstanceKey.toString().replace(",",";");
                temp = temp + objectInstance + ",";
            }
            if (tracer.usingSpecificSourceLocations()){
                String specificSourceLocation = getShortDescription(callMapKey.get(mapKey).specifiedSourceLoc).replace(",",";");
